...
- KDL (unlike serde, but like most other human-readable data formats) doesn't distinguish between different kinds of number, or even between integers and floats. kaydle, therefore, uses a set of parsing rules to parse number as either an `f64`, `i64`, or `u64`, which is then deserialized. The specific rules for this process are covered by kaydle's semver versioning but are documented separately and left deliberately unspecified in this spec.
